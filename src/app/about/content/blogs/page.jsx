...
const Blogs = () => {
    ...
    return (
        <div>
            <h1 className="font-bold text-3xl text-pink-500 text-center mx-2.5 my-10">It is my day blogs section</h1>
            <div className="border-2 border-green-400">
                {
                    blogs.map(data =>
                        <div
                            className="flex flex-col justify-center items-center border mx-3 my-6 p-4 border-amber-400"
                            key={data.id}>
                            <h2 className="font-bold text-center">{data.title}</h2>
                            <Link href={`/about/content/blogs/${data.id}`}
                                className="btn btn-p"
                            >Go Deep</Link>
                        </div>)
                }
